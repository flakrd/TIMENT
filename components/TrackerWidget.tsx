...
const TrackerWidget: React.FC<TrackerWidgetProps> = ({ title = "Rastreador", isRestMode }) => {
  ...
  const renderVitals = () => (
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300 flex-1 overflow-y-auto pr-1">
        {[
          { id: 'coffee', label: 'Cafeína', sub: 'Energía líquida', icon: '☕', val: coffees, color: isRestMode ? 'bg-amber-900/20 border-amber-800' : 'bg-amber-50 border-amber-100', bump: coffeeBump },
          { id: 'water', label: 'Agua', sub: 'Hidratación', icon: '💧', val: water, color: isRestMode ? 'bg-cyan-900/20 border-cyan-800' : 'bg-cyan-50 border-cyan-100', bump: waterBump },
          { id: 'poop', label: 'Descargas', sub: 'Tiempo en el baño', icon: '💩', val: poops, color: isRestMode ? 'bg-stone-900/20 border-stone-800' : 'bg-stone-50 border-stone-100', bump: poopBump }
        ].map(item => (
          <div key={item.id} className={`rounded-2xl p-3 border flex items-center justify-between transition-all duration-300 ${item.color} ${item.bump ? 'scale-[1.02]' : ''}`}>
              <div className="flex items-center gap-3">
                   <div className={`text-2xl filter drop-shadow-sm ${item.bump ? 'animate-pop' : ''}`}>{item.icon}</div>
                   <div className="flex flex-col">
                      <span className={`text-[10px] font-bold uppercase tracking-wide ${isRestMode ? 'text-slate-400' : 'text-slate-800'}`}>{item.label}</span>
                      <span className={`text-xs font-medium ${isRestMode ? 'text-slate-500' : 'text-slate-600'}`}>{item.sub}</span>
                   </div>
              </div>
              <div className="flex items-center gap-2">
                  <button onClick={() => handleUpdate(item.id as any, -1)} className={`w-7 h-7 rounded-full font-bold shadow-sm transition-transform active:scale-90 ${isRestMode ? 'bg-slate-800 text-slate-400' : 'bg-white text-slate-600'}`}>-</button>
                  <span className={`text-lg font-black w-6 text-center ${isRestMode ? 'text-white' : 'text-slate-800'}`}>{item.val}</span>
  ...
  const renderPomodoro = () => (
      <div className="animate-in fade-in slide-in-from-bottom-2 duration-300 h-full flex flex-col min-h-0">
          <div className={`rounded-2xl p-4 border text-center mb-4 shrink-0 transition-all duration-500 ${isRestMode ? 'bg-slate-950 border-slate-800' : 'bg-red-50 border-red-100'} ${pomoFinished ? 'animate-finish-flash ring-2 ring-green-500' : ''}`}>
              <div className={`text-4xl font-black font-mono tracking-wider mb-2 ${pomoFinished ? 'text-green-500' : (isRestMode ? 'text-indigo-400' : 'text-red-600')}`}>
              ...
              <div className="flex justify-center gap-2">
                  ...
                  <button onClick={() => { setIsPomoRunning(false); setPomoFinished(false); setPomoTime(25*60); }} className={`px-3 py-1 rounded-lg border font-bold text-sm ${isRestMode ? 'border-slate-800 text-slate-500' : 'border-slate-200 text-slate-400'}`}>↺</button>
              </div>
          </div>
          <div className="flex-1 flex flex-col min-h-0">
             ...
             <form onSubmit={addTask} className="flex gap-2 mb-3 shrink-0">
                 <input type="text" value={newTaskText} onChange={(e) => setNewTaskText(e.target.value)} placeholder="Nueva tarea..." className={`flex-1 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-1 transition-colors ${isRestMode ? 'bg-slate-950 border-slate-800 text-white focus:ring-indigo-500' : 'bg-slate-50 border-slate-200 focus:ring-indigo-300'}`} />
                 <button type="submit" className={`px-3 rounded-lg font-bold ${isRestMode ? 'bg-indigo-900 text-indigo-300' : 'bg-slate-200 text-slate-600'}`}>+</button>
             </form>
             <div className="flex-1 overflow-y-auto pr-1 space-y-1 min-h-0">
                 {tasks.map(task => (
                     <div key={task.id} className={`flex items-center gap-3 p-2 rounded-lg transition-colors group ${isRestMode ? 'hover:bg-slate-900' : 'hover:bg-slate-50'}`}>
                         <button onClick={() => toggleTask(task.id)} className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${task.completed ? 'bg-green-500 border-green-500 text-white' : (isRestMode ? 'border-slate-700' : 'border-slate-300')}`}>{task.completed && '✓'}</button>
                         <span className={`text-sm flex-1 truncate ${task.completed ? 'text-slate-500 line-through' : (isRestMode ? 'text-slate-200' : 'text-slate-700')}`}>{task.text}</span>
  ...
  return (
    <div className={`rounded-3xl p-6 transition-all duration-500 flex flex-col h-full max-h-[500px] border ${isRestMode ? 'bg-slate-900/80 backdrop-blur-xl border-slate-700/50 text-white shadow-[0_20px_40px_-12px_rgba(0,0,0,0.5)] ring-1 ring-white/10' : 'bg-white/95 backdrop-blur-xl border-white/50 text-slate-900 shadow-[0_20px_40px_-12px_rgba(0,0,0,0.1)] ring-1 ring-slate-900/5'}`}>
      <div className="flex items-center justify-between mb-6 pb-2 border-b border-white/5 shrink-0">
          ...
          <div className={`flex rounded-xl p-1 transition-colors ${isRestMode ? 'bg-slate-950' : 'bg-slate-100'}`}>
